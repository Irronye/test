...
def main_worker(rank, world_size, cfg):
    ...
    encoder_q = build_model(cfg.model)  # online network
    encoder_k = build_model(cfg.model)  # target network
    model = BYOL(encoder_q, encoder_k, **cfg.byol)  # cfg.byol.dim, pred_dim, m
    model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
    model.cuda()
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[cfg.local_rank], find_unused_parameters=True)
    criterion = build_loss(cfg.loss).cuda()  # CosineSimilarity(dim=1)
